...
export default function VideoPreview({youtubeId,seconds,title}:{youtubeId:string;seconds:number;title:string}) {
  ...
  useEffect(()=>{
    ...
    setState('loading');const mount=document.createElement('div');container.replaceChildren(mount);
    ...
    loadYouTubeAPI().then(api=>{
      ...
      instance=new api.Player(mount,{videoId:youtubeId,playerVars:{start:Math.floor(start),autoplay:0,mute:1,playsinline:1,controls:0,rel:0,origin:window.location.origin},events:{
        ...
        onStateChange:e=>{if(disposed)return;if(e.data===1)stopped=false;if(e.data===0){stopped=true;setState('ended');}},
  ...
  return <div className="video-preview" data-preview-state={state} data-start-seconds={start} data-end-seconds={start+PREVIEW_SECONDS}>
