...
export default function VideoPreview({youtubeId,occurrence,title,continuous=false}:{youtubeId:string;occurrence:SearchOccurrence;title:string;continuous?:boolean}) {
  ...
  useEffect(()=>{
    ...
    loadYouTubeAPI().then(api=>{
      ...
      instance=new api.Player(mount,{videoId:youtubeId,playerVars:{start:Math.floor(start),autoplay:0,mute:getPreviewVolume()===0?1:0,playsinline:1,controls:0,rel:0,origin:window.location.origin},events:{
        onReady:e=>{if(disposed)return;player.current=e.target;applyPreviewVolume(e.target);container.querySelector('iframe')?.setAttribute('tabindex','-1');
          ...
          else {e.target.loadVideoById({videoId:youtubeId,startSeconds:start,...(continuous?{}:{endSeconds:start+PREVIEW_SECONDS})});setState('playing');}},
    ...
    const interval=setInterval(()=>{if(player.current){setActualVolume(player.current.getVolume());setMuted(player.current.isMuted());}const time=player.current?.getCurrentTime();if(!continuous&&!stopped&&time!=null&&time>=start+PREVIEW_SECONDS){stopped=true;player.current?.pauseVideo();setState('ended');}},200);
  ...
  },[youtubeId,start,continuous]);
  return <div className="video-preview" data-preview-state={state} data-muted={muted??undefined} data-volume={actualVolume??undefined} data-start-seconds={start} data-observed-start-seconds={observedStart??undefined} data-end-seconds={continuous?undefined:start+PREVIEW_SECONDS} data-continuous={continuous}>
