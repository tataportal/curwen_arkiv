...
export default function CommercialDemo() {
 ...
 const who=PEOPLE.find(p=>p.id===person),label=connection||query||who?.fullName||'Todos los personajes';
 ...
 function restore() {
  ...
  if(shared){setPerson(shared.person);setInput(PEOPLE.find(p=>p.id===shared.person)?.name??'');setQuery('');setConnection('');setListFilter([shared.id]);setListPath([PEOPLE.find(p=>p.id===shared.person)?.name??'',shared.topics.some(t=>t.label===p.get('topic'))?p.get('topic')!:shared.title]);setActiveVideo(shared.id);requestAnimationFrame(()=>moments.current?.scrollIntoView({behavior:'instant'}));}
 }
 ...
 function showPreview(next:DemoSelection) {
  ...
  if(preview?.expanded||(preview?.label===next.label&&JSON.stringify(preview.path)===JSON.stringify(next.path)))return;
