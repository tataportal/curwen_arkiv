...
import {registerMascotClick} from '@/lib/mascot-clicks';
...
export default function PixelMascot({paused=false,onSecret}:{paused?:boolean;onSecret?:()=>void}) {
 ...
 useEffect(()=>{if(paused){burst.current=[];setSpeaking(false);if(timer.current)clearTimeout(timer.current);}},[paused]);
 function speak(){if(timer.current)clearTimeout(timer.current);const next=registerMascotClick(burst.current,Date.now());burst.current=next.clicks;if(next.triggered&&onSecret){setSpeaking(false);onSecret();return;}setClicks(n=>n+1);setSpeaking(true);timer.current=setTimeout(()=>setSpeaking(false),3800);}
