...
export type Evidence = { publishedAt?:string|null; evidenceTokens?:SpeechToken[]; processed?:ProcessedMomentView; youtubeId:string; title:string; seconds:number; endSeconds?:number; text:string; precision:'cue'; chunkId:string; momentId:string; occurrence:SearchOccurrence; occurrences:SearchOccurrence[]; fullContext:SpeechSentence[]; longMoment:boolean };
...
export function resultEvidence(episode:RetrievalEpisode,moment:DiscussionMoment,term?:string):Evidence[] {
  ...
  return [{youtubeId:episode.videoId,title:episode.title,publishedAt:episode.publishedAt,seconds:o.cue_start_seconds,endSeconds:moment.endSeconds,
